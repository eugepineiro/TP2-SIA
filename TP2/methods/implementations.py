...
def fill_all(old_generation,children,k,n,replacement_a,replacement_b,replacement_prob,generation, threshold):
...
def fill_parent(old_generation,children,k,n,replacement_a,replacement_b,replacement_prob,generation, threshold):
    if k > n:
        selected_children1 = selection(replacement_a,children,math.floor(n*replacement_prob),len(children),generation, threshold)
        selected_children2 = selection(replacement_b,children,math.ceil(n*(1-replacement_prob)),len(children),generation, threshold)
        return selected_children1 + selected_children2
    elif k == n:
        return children
    selected_old1 = selection(replacement_a,old_generation,math.floor((n-k)*replacement_prob),len(old_generation),generation, threshold)
    selected_old2 = selection(replacement_b,old_generation,math.ceil((n-k)*(1-replacement_prob)),len(old_generation),generation, threshold)
