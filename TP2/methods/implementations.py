...
def fill_all(old_generation,children,k,n,replacement_a,replacement_b,replacement_prob,generation):
    ...
    first = selection(replacement_a,join,math.ceil(n*replacement_prob),len(join),generation)
    second = selection(replacement_b,join,math.floor(n*(1-replacement_prob)),len(join),generation)
...
def fill_parent(old_generation,children,k,n,replacement_a,replacement_b,replacement_prob,generation):
    if k > n:
        selected_children1 = selection(replacement_a,children,math.ceil(n*replacement_prob),len(children),generation)
        selected_children2 = selection(replacement_b,children,math.floor(n*(1-replacement_prob)),len(children),generation)
        return selected_children1 + selected_children2
    elif k == n:
        return children
    selected_old1 = selection(replacement_a,old_generation,math.ceil((n-k)*replacement_prob),len(old_generation),generation)
